...
class IDSServoDriver(can.listener.Listener):
    ...
    def on_message_received_position(self, msg:can.Message) -> None:
        ...
        if msg.data[1] == FunctionCode.OTO_WRITE_RETURN.value:
            ...
            print("Read command sent and received")
        ...
        elif msg.data[1] == FunctionCode.OTO_READ_ERROR.value:
    ...
    def read_actual_position(self, wait:bool=False):
        """Set control mode to positional mode."""
        data = [
            self.group_id, # Driver's group ID
            FunctionCode.OTO_READ_SEND.value, # Function code
            RegisterAddress.READ_CURRENT_POSITION_MSB.value, # Register 2 (Servo start/stop)
            0x00, # N/A
            0x00, # Stop servo
            RegisterAddress.READ_CURRENT_POSITION_LSB.value, # Register 2 (Control mode selection)
            0x00, # N/A
        ...
        self.on_message_received_position()

    def monitor_position(self, instance, threshold=5, interval=0.5):
        """
        ...
        """
        prev_pos = instance.read_actual_position()
        while True:
            time.sleep(interval)
            actual_pos = instance.read_actual_position()
