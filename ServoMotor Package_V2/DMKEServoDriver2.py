...
class DMKEServoDriver2_V1:
  ...
  ################################################################################################3
    ...
    def write_speed(self, speed):
        ...
        try:
            ...
            if speed >= 0:
                ...
                speed_data = 4294967294 - int(((abs(speed)/0.1)/60)*10000) 
                data_bytes = speed_data.to_bytes(4, byteorder='little', signed=True)  # Convert to bytes
    ...
    def operating_current(self, op_current):
        """Set current rise rate of motor
        ...
        try:
            ...
            data_bytes = op_current.to_bytes(4, byteorder='little')  # Convert to bytes
            current_rise_rate.raw = data_bytes
    ...
    def node_timeout_protection(self, timeout:int, factor:int):
        ...
        try:
            ...
            data_bytes = timeout.to_bytes(4, byteorder='little', signed=False) # Timeout in ms
            ...
            data_bytes2 = factor.to_bytes(4, byteorder='little', signed=False) # factor
